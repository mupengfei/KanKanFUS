...
(function(){
	...
	SWFProvider.prototype = {
		...
		renderUI : function(a) {
			...
			var swfURLtem = this.get("swfURL")+"?rd="+ new Date().getTime();
			alert(swfURLtem);
			this.swfReference = new SWFReference(b, swfURLtem, {
						version : "10.0.45",
						fixedAttributes : {
							wmode : "transparent",
	...
	StreamUploader.prototype = {
		...
		uploadEventHandler: function(event){
			console.log("uploadEventHandler"  );
	...
	Main.prototype = {
		...
		upload : function(index) {
			...
			tokenUrl = tokenUrl+"&jsonpCallback=?";
			alert("call-:"+tokenUrl);
			$.getJSON(tokenUrl,  
				function(data){  
					...
					try {
						...
						if (token) {
							...
							var msg = "\u521B\u5EFA\u4E0A\u4F20\u4EFB\u52A1\u5931\u8D25[tokenURL=" + self.get("tokenURL") + "],\u72B6\u6001\u7801:" ;
